---
name: kisou
description: Scaffold a new project's standard structure, or retrofit (migrate) it onto an existing repo — README / CONTRIBUTING / CLAUDE / a slim AGENTS.md, the docs/ document-management system, and optional empty script files; project names follow the chosen `case` convention (`snake_case` or `PascalCase`, with `docs → Documents` / `src → Source` etc. for the latter). Triggers on `起草して` or `kisouして`, with an optional `scaffold` / `migrate` mode argument. Never touches src/ or tests/.
---
